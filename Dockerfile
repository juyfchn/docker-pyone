...
COPY docker-entrypoint.sh healthcheck.sh aria2c aria2.conf supervisord.conf requirements.txt /
...
RUN \
    ...
    rm /etc/PyOne/requirements.txt && \                                                         
    mv /requirements.txt /etc/PyOne/ && \
